...
module.exports = {
	...
	getJournalById: function(req, res) {
	...
	getPublicationById: function(req, res) {
	...
	postPublicationById: function(req, res) {
	...
	postUserLogin: function(req, res) {
		...
		var onSuccess = function(users) {
			if(users.length === 1) {
				var token = jwt.sign(users[0].id, config.secretToken, { expiresInMinutes: 60 });
