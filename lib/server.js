...
app.get('/disciplines.json', function(req, res) {
    ...
    var result = dbManager.getDisciplines('user', 'password', function(disciplines) {
        var jsonDis = [];
...
app.get('/journals.json', function(req, res) {
    var dbManager = new DBManager('wilma.vub.ac.be', 'se2_1415');
    dbManager.getJournals('user', 'password', function(journals) {
        var jsonJou = [];
...
app.post('/users/:id/publication.json', function(req,res){
  ...
  if(parsed.type == "Journal"){
    ...
    dbManager.postJournalPublication('user', 'password', function(respons){
      res.status(200).end();
    }, journal);
  } else if (parsed.type == "Proceeding"){
    var proceeding = new ProceedingPublication(parsed);
    dbManager.postProceedingPublication('user', 'password', function(respons){
      res.status(200).end();
    },proceeding)
  } else {
...
app.listen(8080);
>>>>>>> 5baea2dc2caebcb2ce7d015f15d61d5e5c687d26
