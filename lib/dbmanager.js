 ...
 * @param {Discipline} discipline - The Discipline that will be updated.
 ...
 * Because queries are asynchronous, a callback is needed for when the query finished.
 * @param {Discipline} discipline - The Discipline that will be added.
 * @param {function} next - The callback that will be executed at the end of the query.
 * @public
 */
  ...
  var onSucces = function(res) {
    res.on('row', function(row){
      for(var i = 0; i < publication.authors.length; i++){
        ...
        var queryParams = {publication_id: row.id, publication.authors[i].id};
        manager.executeQuery(query, queryParams);
      }

      for(var i = 0; i < publication.keywords.length; i++) {
        var query = 'INSERT INTO publication_has_keyword VALUES (:publication_id , :keyword)';
        var queryParams = {publication_id: row.id, publication.keywords[i].id};
        manager.executeQuery(query, queryParams);
      }
    }
  }

  var insertSucces = function(res){
    var query = 'SELECT id FROM publication WHERE title=:title';
    var queryParams = {title:publication.title}
    manager.executeQuery(query, queryParams, onSuccess);
  }

  this.executeQuery(query, queryParams, insertSucces);
}
