...
DBManager.prototype.postDiscipline = function(jsonObj, next) {
    ...
    var queryOptions = {name: jsonObj.name};
 ...
 */
 DBManager.prototype.postJournal = function(jsonObj, next) {
     ...
     var lastQuery;
     var id = null;
     var insertJournalQuery = {
         query: 'INSERT INTO journal (name, rank) VALUES (:name, :rank)',
         queryParams: {name: jsonObj.name, rank: jsonObj.rank},
         onEnd: function(info) {
             id = info.insertId;
         }
     }
     lastQuery = insertJournalQuery;
     queries.push(insertJournalQuery);

     for(var i = 0; i < jsonObj.disciplines.length; i++) {
        var insertDisciplinesQuery = {
            query: 'INSERT INTO journal_has_academic_discipline VALUES (:journal_id, :discipline_id)',
            queryParams: {journal_id: id, discipline_id: jsonObj.disciplines[i].id}
        };
        lastQuery = insertDisciplinesQuery;
        queries.push(insertDisciplinesQuery);
    }
    var onEnd = function(info) {
        next(info.insertId);
    };
...
DBManager.prototype.postPublication = function(jsonObj, next){
    ...
    var insertPublicationQuery = {
        ...
        queryParams: {publication_type: jsonObj.type, title: jsonObj.title,
    ...
    for(var i = 0; i < jsonObj.authors.length; i++) {
        var insertAuthorQuery = {
            ...
            queryParams: {publication_id: id, person_id: jsonObj.authors[i].id},
        };
    ...
    for(var i = 0; i < jsonExp.keywords.length; i++) {
        var insertKeywordQuery = {
            ...
            queryParams: {publication_id: id, keyword: jsonObj.keywords[i].id}
        };
