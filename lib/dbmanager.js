 ...
 */
DBManager.prototype.executeMultipleQueries = function(queryArr) {
    var manager = this;
    var execMultQueries = function(queries) {
        console.log('queries size ' + queries.length);
        console.log('all queries : ' + JSON.stringify(queries));
        var currentQuery = queries[0];
        console.log('current query : ' + currentQuery.query);
        if(queries.length === 1) {
            manager.executeQuery(currentQuery.query, currentQuery.queryParams, currentQuery.onSuccess, currentQuery.onError, currentQuery.onEnd);
        }
        else {
            var onEnd = function() {
                if(typeof currentQuery.onEnd !== 'undefined') {
                    currentQuery.onEnd();
                }
                queries = queries.splice(-1,1);
                console.log('next query to be executed : ' + queries[0].query);
                execMultQueries(queries);
            };
            manager.executeQuery(currentQuery.query, currentQuery.queryParams, currentQuery.onSuccess, currentQuery.onError, onEnd);
        }
    };
    console.log('queryArr : ' + JSON.stringify(queryArr));
    execMultQueries(queryArr);
};
    ...
    var onSuccess = function(res) {
        res.on('end', function(info) {
            ...
            for(var i = 0; i < jsonObj.disciplines.length; i++) {
                q  += 'INSERT INTO journal_has_academic_discipline (journal_id, academic_discipline_id) ' +
                      'VALUES (:journal_id, :discipline_id_' + i + ');';
