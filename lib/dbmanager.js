...
DBManager.prototype.getIter = function(queries, options, classObj, previous, next) {
    ...
    var onEnd = function() {
        ...
        for(var toAdd in nextIteration) {
            //see if there is no object with id of future query
            var fr = previous[reprGet[1].repr.reprTables[0]];
            if(fr[toAdd] === undefined) {
                nextQuery += nextIteration[toAdd];
            }
            else {
                alreadyObj.push(fr[toAdd]);
            }
        }
        ...
        var idToObj = function(r) {
            var nextIterRes = {};
            //reference objects by their id
            for(var i = 0; i < r.length; i++) {
                nextIterRes[r[i][reprGet[1].id.fieldName]] = r[i]; 
            }
            //only replace if there are objects to replace with
            if(r.length > 0) {
                //replace each foreign id with its foreign object
                for(var i = 0; i < resArray.length; i++) {
                    var replace = resArray[i][reprGet[1].foreign.name];
                    if(reprGet[1].foreign.multiple) {
                        ...
                        resArray[i][reprGet[1].foreign.name] = nextIterRes[replace];
                    }
                }
            } 
        };
        ...
        //recursively apply getIter to foreign objects to fetch
        else {
            manager.getIter(nextQuery, undefined, reprGet[1].repr, previous, after);
        }
    };
...
DBManager.prototype.getDiscipline = function(params, next) {
    var t = dbscheme.user.select(dbscheme.user.star())
                                     .from(dbscheme.user)
                                     .toQuery();
    console.log('dbscheme test : ' + t.text);
    ...
    //var jour = {name: 'test', rank: 1, disciplines: [{id: 1}, {id:2}]};
    var jour = {id: 3};
    console.log(linker.queryGet(jour, linker.journalRepr));
...
var jsonToDB = function(jsonObj){
    return _.transform(jsonObj, function(result, value, key){
        result[key.toUnderscore()] = value;
    })
}
...
DBManager.prototype.getUser = function(params, next) {
    ...
    var onUserSuccess = function(res) {
        var resultUsers = [];
        //Used for checking if last user has been processed (if this isn't used, there are problems with nested callbacks and this on('end') is immediately executed)
        //TODO: find better solution
        var ctr = 0;
        var total = 0;
        res.on('row', function(row){
            var onPersonSuccess = function(PersonRes) {
                var person = new Person(PersonRes[0]);
                row['person'] = person;
                var user = new User(row);
                resultUsers.push(user);
                console.log('ctr: ' + ctr + ', total: ' + total);
                if(++ctr >= total) {
                    console.log('finished');
                    next(resultUsers);
                }
            }
            dbManager.getPerson({'id': row.person_id}, onPersonSuccess);
        }).on('end', function(info) {
            total = info.numRows;
        });
...
/**
* This function posts a Person to the database
* @param {object} jsonObj Object with Person information
* @param {Function} next What to to after the values are inserted in the database
*/
DBManager.prototype.postPerson = function(jsonObj, next) {
    var query = dbscheme.person.insert(dbscheme.person.first_name.value(jsonObj.firstName),
                                       ...
                                       dbscheme.person.part_of_affiliation_id.value(1),
                                       dbscheme.person.picture.value(jsonObj.picture))
    .toQuery();
    var onSuccess = function(res) {
        res.on('end', function(info) {
            next(info.insertId);
...
DBManager.prototype.postUser = function(jsonObj, next) {
    ...
    var afterPersonId = function(personId) {
        var query = dbscheme.user.insert(dbscheme.user.person_id.value(personId),
                                         dbscheme.user.email_address.value(jsonObj.email),
