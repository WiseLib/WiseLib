...
DBManager.prototype.getIter = function(queries, options, classObj, next) {
    var manager = this;
    var results = {};
    var resArray = [];
    var nextIteration = {};
    ...
    var onSuccess = function(res) {
        ...
        res.on('row', function(row) {
            ...
            if(results[objId] === undefined) {
                ...
                //current = row[reprGet[0].fieldName];
                if (reprGet.length > 1) {
                    //create empty array to store future foreign ids
                    results[objId][reprGet[1].foreign.name] = [];
                }
            }
            if (reprGet.length > 1) {
    ...
    var onEnd = function() {
        ...
        //recursively apply getIter to foreign objects to fetch
            ...
            manager.getIter(nextQuery, undefined, reprGet[1].repr, after);
        }
    };
...
DBManager.prototype.get = function(jsonObj, classObj, next) {
    ...
    this.getIter(query, jsonObj, classObj, next);
};
