 ...
 */
 DBManager.prototype.postDiscipline = function(jsonObj, next) {
 	var query = 'INSERT INTO academic_discipline (name, part_of_academic_discipline_id) VALUES (:name, :parentId)';
 	var onSuccess = function(res) {
 ...
 */
 DBManager.prototype.deleteDiscipline = function(discipline, next) {
    var onSuccess = function(res) {
 	...
 	var result = this.executeQuery('DELETE FROM academic_discipline WHERE id=:id', {id: discipline.id},onSuccess);
 };
 ...
 * @public
 */
 DBManager.prototype.postProceeding = function(jsonObj, next) {
...
DBManager.prototype.postUser = function(jsonObj, next) {
	...
	if (jsonObj.personId) {
		afterPersonId(jsonObj.personId);//register user immediatly
	} else {
		this.postPerson(jsonObj, afterPersonId);//register person first, register user in callback
	}
};
//Exports
