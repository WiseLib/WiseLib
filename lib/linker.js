...
personRepr.reprInsert = [[personRepr.firstName, personRepr.lastName, personRepr.picture]];
personRepr.reprGet = [personRepr.id];
personRepr.reprNew = function(row) {
	var person = new Person({firstName: row.first_name, lastName: row.last_name, id: row.id, picture: row.picture});
	return person;
};
...
//publication
var publicationRepr = {
	reprTables: ['publication', 'publication_written_by_person']
};
publicationRepr.id = {
...
publicationRepr.authors = {
	table: publicationRepr.reprTables[1],
	fieldName: 'person_id',
	name: 'authors',
	multiple: true,
	foreign: 'publication_id'
};
publicationRepr.reprInsert = [[publicationRepr.title, publicationRepr.numberOfPages, publicationRepr.year,
publicationRepr.url, publicationRepr.abstract, publicationRepr.uploader],
['publication_id', publicationRepr.authors]];
publicationRepr.reprGet = [publicationRepr.id, {foreign: publicationRepr.uploader, repr: userRepr, id: userRepr.id},
{foreign: publicationRepr.authors, repr: personRepr, id: personRepr.id}];
publicationRepr.reprNew = function(row) {
	var params = {title: row.title, numberOfPages: row.nr_of_pages, year: row.published_in_year, url: row.url, abstract: row.summary_text};
	params.authors = []; //TEMP
	params.disciplines = []; //TEMP
...
var queryUpdate = function(params, classObj) {
	var query = 'UPDATE ';
	query += classObj.reprTables[0];
	query += ' SET ';
	for(var param in params) {
		query += classObj[param].fieldName + '=' + ':' + param;
		query += ', ';
	}
...
var queryGet = function(params, classObj) {
	...
	for(var e = 1; e < classObj.reprGet.length; e++) {
		var makeJoin = false;
		for(var i = 1; i < classObj.reprTables.length; i++) {
			makeJoin = makeJoin || classObj.reprGet[e].foreign.table === classObj.reprTables[i];
		}
		if(makeJoin) {
			var currField = classObj.reprGet[e];
			query += ' INNER JOIN ';
...
var queryDelete = function(params, classObj) {
...
var queryOptions = function(params, classObj) {
	...
	for(var variable in params) {
		console.log('args: ' + JSON.stringify(params[variable]));
		query += query === '' ? ' WHERE ' : ' AND '; //Add WHERE if begin of query, AND otherwise
		console.log('typeof: ' + typeof(params[variable]));
		if(typeof(params[variable]) === 'object') {
			query += classObj[variable].fieldName + ' IN (' + params[variable].join(',') + ')';
		} else {
