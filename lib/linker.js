...
personRepr.reprNew = function(row) {
	var person = new Person({firstName: row.first_name, lastName: row.last_name, id: row.id});
	return person;
};
...
publicationRepr.reprNew = function(row) {
	var params = {title: row.title, numberOfPages: row.nr_of_pages, year: row.year, url: row.url, abstract: row.summary_text};
	params.authors = []; //TEMP
