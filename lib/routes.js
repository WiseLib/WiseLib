...
module.exports = function(app) {
    ...
    .get(function(req, res) {
        dbManager.get({id: req.params.id}, linker.personRepr, function(persons) {
            if(persons.length !== 1) {
                res.sendStatus(404);
            } else {
                res.json(persons[0]);
    ...
    .get(function (req, res) {
        ...
        dbManager.get(params, linker.personRepr, function(persons) {
            console.log('params for person search: ' + JSON.stringify(params));
            res.json({
    ...
    .get(function(req, res) {
        dbManager.get({authors: [req.params.id]}, linker.publicationRepr, function(publications) {
            res.json({
    ...
    .get(function(req, res) {
        dbManager.get({id: req.params.id}, linker.publicationRepr, function(publications) {
            if (publications.length === 1) { //We should receive exactly one publication from the database
                res.json(JSONManager.generatePublication(publications[0]));
            } else {
    ...
    .delete(auth, function(req, res) {
        dbManager.delete({id: req.params.pubId}, linker.publicationRepr, function() {
            res.end();
        });
    });
